...
if [ ! -d "$COMFYUI_DIR" ] || [ ! -d "$VENV_DIR" ]; then
    ...
    # Create venv with access to system packages (torch cu129, numpy, etc. pre-installed in image)
    if [ ! -d "$VENV_DIR" ]; then
        cd "$COMFYUI_DIR"
        ...
        echo "Base packages (torch cu129, numpy, etc.) available from system site-packages"
